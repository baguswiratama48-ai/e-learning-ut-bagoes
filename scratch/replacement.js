  ...
  if (!isLandscape) {
    return (
      <div className="fixed inset-0 z-[100] bg-primary text-white flex flex-col items-center justify-center p-10 text-center">
        ...
        <p className="opacity-70 font-medium max-w-xs">Gunakan mode <strong>Lanskap (Miring)</strong> agar Mind Map terlihat jelas.</p>
      </div>
    );
  }
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-3 mb-8">
              {[
                ...
                { n: '2', icon: 'touch_app', text: 'Tekan & geser bubble kata ke lingkaran cabang yang tepat (Drag & Drop).' },
                { n: '3', icon: 'info', text: 'Setiap jawaban benar memunculkan info singkat.' },
                { n: '4', icon: 'send', text: 'Jika semua terpasang, tekan KIRIM dan screenshot hasilnya.' }
              ].map(s => (
      ...
      <div className="fixed inset-0 z-[60] bg-black bg-opacity-70 backdrop-blur-md flex items-center justify-center p-4">
        <div className="bg-white rounded-[3rem] p-8 md:p-10 max-w-2xl w-full shadow-2xl animate-in zoom-in duration-300">
          ...
          <div className="grid grid-cols-1 gap-4 mb-6">
            {[
              { id: 'A', label: 'Layanan Mediasi & Asas Keterbukaan', correct: false, desc: 'Mediasi untuk konflik antar pihak.' },
              { id: 'B', label: 'Konseling Individual & Asas Kerahasiaan', correct: true, desc: 'Tepat! Menjaga privasi siswa.' }
            ].map(opt => (
              <button
                key={opt.id}
  ...
  return (
    <div className="relative w-full bg-white rounded-[3rem] overflow-hidden border-4 border-slate-100 shadow-2xl flex flex-col" style={{ minHeight: '580px' }}>
      ...
      <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <p className="text-sm font-black text-primary">{placedCount} / {totalItems}</p>
          </div>
      ...
      <div className="flex-1 relative flex items-center justify-center overflow-hidden" style={{ minHeight: '380px' }}>
        <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
          {[[-150, 20], [150, 20], [0, -160]].map(([x, y], i) => (
            <line key={i} x1="50%" y1="50%" x2={`calc(50% + ${x}px)`} y2={`calc(50% + ${y}px)`} stroke="#e2e8f0" strokeWidth="3" strokeDasharray="8,6" />
          ))}
        </svg>
        <div className="absolute w-24 h-24 bg-primary rounded-full flex items-center justify-center text-white font-black text-center text-xs p-3 shadow-2xl z-10 ring-8 ring-blue-50 bg-opacity-100">MIND MAP<br />BK</div>
        {MIND_MAP_DATA.zones.map((zone, idx) => {
          ...
          return (
            <div
              ...
              style={{ ...pos, zIndex: 5 }}
            >
              <div className={`px-3 py-1 rounded-xl ${zone.color} text-white text-[8px] font-black uppercase tracking-wider shadow-md mb-2`}>{zone.label}</div>
              <div className="flex flex-wrap justify-center gap-1 px-2 max-w-full">
                {placedInZone.map(([iid]) => {
                  ...
                  return <span key={iid} className={`px-2 py-0.5 rounded-lg ${zone.color} text-white text-[8px] font-black shadow animate-in zoom-in duration-300`}>{item.label}</span>;
                })}
              </div>
            </div>
          );
      ...
      <div className="bg-slate-900 px-6 py-5 flex flex-col items-center gap-3">
        ...
        {allDone && (
          <button onClick={() => setGameState('CHALLENGE')} className="mt-1 px-10 py-3 bg-yellow-400 text-slate-900 rounded-2xl font-black text-sm hover:scale-105 active:scale-95 transition-all shadow-xl flex items-center gap-2">
            <span className="material-symbols-outlined">emoji_events</span> LANJUT TANTANGAN AKHIR
          </button>
        )}
      </div>
