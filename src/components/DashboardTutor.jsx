              ...
              <tbody>
                {forumActivityReport.rows.map((row, idx) => (
                  <tr key={row.email} className="bg-white border border-slate-100 shadow-sm rounded-3xl hover:shadow-md transition-all">
                    ...
                    <td className="px-4 py-4 border-y bg-white"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-sm shrink-0">{row.name.charAt(0)}</div><div><p className="font-black text-slate-800 text-sm leading-none mb-1">{row.name}</p><p className="text-[10px] text-slate-400 font-medium">{row.nim}</p><p className="text-[9px] text-indigo-400 font-bold">{row.email}</p></div></div></td>
